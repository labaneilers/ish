...
var getFileDataList = function (filePaths, rootDir) {
    if (!filePaths || filePaths.length === 0) {
        ...
        filePaths = langDir.getAllMatchingWwwPaths(rootDir)
    ...
    return filePaths
        ...
        // Only look at images that are controlled by imagesettings
        .filter(imagesettings.isImageSettingsImage)

        // For each of the file paths, expand to all corresponding www* paths
        .map(langDir.getAllMatchingWwwPaths)
        .flatten()
