...
function processFile(fileData, options, stdout, stderr) {
    ...
    if (fileData.imageSettings.type == "file") {
        return Q("imagesetings.xml file exists: " + fileData.imageSettings.format);
    }

    return helper.process(fileData.fullPath, options.savetemp)
        .then(
            function (result) {
                if (!options.verbose) {
                ...
                if (normalizeFormat(fileData.imageSettings.format) != result.bestFormat) {
                    stdout.write("SHOULD CHANGE TO: " + result.bestFormat + "\n");
